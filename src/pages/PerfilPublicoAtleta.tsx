...
export default function PerfilPublicoAtleta() {
  ...
  const uid = id; // pode ser undefined em tempo de tipo
  ...
  async function toggleFollow() {
    ...
    if (isSeguindo) {
      await unfollowUser(usuarioAtual, uid);
      setIsSeguindo(false);
      setFollowersCount((c) => Math.max(0, c - 1));
    } else {
      await followUser(usuarioAtual, uid);
      setIsSeguindo(true);
  ...
  const clubeAtual =
    atleta.experiences?.find((e) => e.current) ||
    atleta.experiences?.slice(-1)[0];
          ...
          <section className="bg-gradient-to-br from-gray-800/90 via-gray-900/90 to-black/90 border border-gray-700/70 rounded-3xl overflow-hidden shadow-xl">
            ...
            <div className="p-6 sm:p-8 -mt-16 flex flex-col sm:flex-row gap-6 sm:gap-8 items-start sm:items-end">
              ...
              <div className="flex-1 w-full">
                ...
                {/* Resumo curto */}
                <p className="text-gray-200 text-sm leading-relaxed mb-4 mt-4">
                  {atleta.bio
                    ? atleta.bio
