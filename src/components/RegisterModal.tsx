...
import { X, Mail, Lock, User, AlertCircle, Loader2, Users, Building2, GraduationCap, Briefcase, Award } from 'lucide-react';
import { registerWithEmail, loginWithGoogle, createGoogleUserProfile } from '../firebase/auth';
...
export default function RegisterModal({ isOpen, onClose, onSwitchToLogin }: Props) {
  ...
  const profiles = [
    { id: 'atleta' as ProfileType, name: 'Atleta', icon: Users },
    { id: 'clube' as ProfileType, name: 'Clube', icon: Building2 },
    { id: 'treinador' as ProfileType, name: 'Treinador', icon: GraduationCap },
    { id: 'agente' as ProfileType, name: 'Agente', icon: Briefcase },
    { id: 'patrocinador' as ProfileType, name: 'Patrocinador', icon: Award }
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await registerWithEmail(email, password, name, selectedProfile as ProfileType);
  ...
  const handleGoogleRegister = async () => {
    ...
    if (!selectedProfile) {
      setError('Por favor, selecione um tipo de perfil antes de continuar com Google');
      return;
    }
    ...
    if (result.success && result.user) {
      // Se for novo usuário, criar perfil
      if (result.isNewUser) {
        await createGoogleUserProfile(result.user, selectedProfile as ProfileType);
  ...
  const getErrorMessage = (error: string) => {
    if (error.includes('email-already-in-use')) return 'Este email já está cadastrado';
    if (error.includes('invalid-email')) return 'Email inválido';
    if (error.includes('weak-password')) return 'Senha muito fraca. Use no mínimo 8 caracteres';
      ...
      <div className="relative bg-gray-900 rounded-2xl p-8 max-w-2xl w-full border border-white/10 shadow-2xl my-8 animate-fade-in">
            ...
            {profiles.map((profile) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-xs font-semibold ${selectedProfile === profile.id ? 'text-orange-500' : 'text-gray-400'}`}>
                    {profile.name}
                  </span>
                </button>
              );
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <label htmlFor="terms" className="text-sm text-gray-400">
              Aceito os <a href="#" className="text-orange-500 hover:text-orange-400">Termos de Uso</a> e a <a href="#" className="text-orange-500 hover:text-orange-400">Política de Privacidade</a>
            </label>
          </div>
        ...
        <div className="text-center mt-6">
          <p className="text-gray-400">
            Já tem uma conta? <button onClick={onSwitchToLogin} className="text-orange-500 font-semibold hover:text-orange-400 transition-colors">Entrar</button>
          </p>
        </div>
      </div>
    </div>
